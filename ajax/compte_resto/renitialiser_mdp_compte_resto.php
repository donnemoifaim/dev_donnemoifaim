...
if(!empty($_POST['lien_unique_renitialisation']))
{
	...
	if(!empty($_POST['nouveau_mdp_compte_resto']))
	{
		if(strlen($_POST['nouveau_mdp_compte_resto']) > 5)
		{
			$nouveau_mdp = securise_mdp_compte_resto($_POST['nouveau_mdp_compte_resto']) ;
			
			// Cherchons le compte qui à appelé ce lien unique et changons la variable mdp par la nouvelle
			$requete_user = $bdd->prepare('UPDATE client SET ancien_mdp = mdp, mdp = :mdp_nouveau WHERE ancien_mdp = :lien_unique') ; 
			...
			{
				echo '<span>Lien de réinitialisation de mot de passe erroné. Si le problème persiste, contactez-nous.</span><br />' ; 
			}
		}
		else
		{
